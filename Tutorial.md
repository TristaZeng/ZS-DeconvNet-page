...
- `./dataset` is the default path for training data generation and test data
  ...
  - `./dataset/test_LLS_3D`includes demo stack of Lamp1 to test ZS-DeconvNet 3D
  - `./dataset/test_confocal_3D`includes demo stack of Microtubules to test ZS-DeconvNet 3D
...
If you would like to use our provided augmented training datasets in the folder `ZS-DeconvNet/augmented_datasets/`, you can skip this part.
...
+ Run `ZS-DeconvNet/data_augment_recorrupt_matlab/DataAugmFor2D.m` in MATLAB to generate 2D training data. Re-corruption is embedded in the process of augmentation. 

+ The augmented training datasets will be saved to the folder `ZS-DeconvNet/your_augmented_datasets/WF_2D`.

+ The default option is to use the  provided datasets in the folder `ZS-DeconvNet/datasets/WF_2D`to generate training data. But you can use your own data or download more data from our [shared datasets](...). If you want to use your own data or download other data, organize the data in the same way we do, or change the data loading part in the MATLAB code.
...
+ Run `ZS-DeconvNet/data_augment_recorrupt_matlab/DataAugmFor3D.m` to generate 3D training data. Re-sampling is embedded in the process of augmentation. 

+ The augmented training datasets will be saved automatically to the folder `ZS-DeconvNet/your_augmented_datasets/LLS_3D`.

+ The default option is to use the provided datasets in the folder `ZS-DeconvNet/datasets/LLS_3D` to perform augmentation and re-sampling, but you can use other data. 
...
+ Change the data paths in `ZS-DeconvNet/train_inference_python/train_demo_2D.sh`or `train_inference_python/train_demo_3D.sh`. 

+ Run it in your terminal.

+ The result wills be saved to `ZS-DeconvNet/train_inference_python/saved_models/`.

+ Run `tensorboard --logdir [save_weights_dir]/[save_weights_name]/graph` to monitor the training process via tensorboard if you want.

If you would rather just try out the training code and not generate any data, you could run `train_demo_2D.sh` or `train_demo_3D.sh`directly, for the default data paths points to the augmented training datasets we have prepared for you.

+ Notice: the padded size of training data should be the multiple of $2^{conv\_block\_num}$, to be compatible with the 2D U-net structure. Be careful if you are changing the parameters `input_x`, `input_y` or `insert_xy`.

<h3 style="color:white;">3.3 Inference Demo</h3>

If you have trained a network yourself and want to test it:

+ Change the model weight paths in `ZS-DeconvNet/train_inference_python/infer_demo_2D.sh` or `ZS-DeconvNet/train_inference_python/infer_demo_3D.sh` accordingly. 

+ Run it in your terminal.

+ The output will be automatically saved to the folder where you load weights. For example, if you load weights from `ZS-DeconvNet/train_inference_python/saved_models/.../weights_40000.h5`, then the output will be saved to `ZS-DeconvNet/train_inference_python/saved_models/.../Inference/`.

+ The default option is to use demo test datasets in the folder `ZS-DeconvNet/datasets/test_WF_2D`, `ZS-DeconvNet/datasets/test_confocal_3D` and `ZS-DeconvNet/datasets/test_LLS_3D`, but you can use other data.

Otherwise:

+ We have provided saved models in the folder` ZS-DeconvNet/train_inference_python/saved_models/`, and they are the default loading weights paths.

+ Run `ZS-DeconvNet/train_inference_python/infer_demo_2D.sh` or `ZS-DeconvNet/train_inference_python/infer_demo_3D.sh` in your terminal.

+ The 2D WF output will be automatically saved to the folder `ZS-DeconvNet/train_inference_python/saved_models/WF_2D_560_beta1_0.5-1.5_beta2_10-15_alpha1-2_SegNum20000_twostage_Unet_Hess0.02/Inference`, 3D confocal output to `ZS-DeconvNet/train_inference_python/saved_models/Confocal_3D_488_twostage_RCAN3D_upsample/Inference/`, and 3D LLS output to `ZS-DeconvNet/train_inference_python/saved_models/LLS_3D_488_Zsize5_Xsize48_fromMRC_twostage_RCAN3D_Hess0.1_MAE_up/Inference`.

+ Notice: If you are using image segmentation and fusion, which may be needed when the test image is too large and the memory runs out, please make sure `input_x-overlap_x` is the multiple of `seg_window_x-overlap_x`, or the image fusion will go wrong. The same caution is needed when dealing with y or z directions.
...
+ Copy `ZS-DeconvNet/Fiji-plugin/jars/*` `ZS-DeconvNet/Fiji-plugin/plugins/*` to your root path of Fiji `/*/Fiji.app/`.
...
| PercentileBottom, PercentileTop | 0,100   | These two hyper-parameters are valid only when you tick NormalizeInput, under which circumstance the image or stack $x$ will be normalized to $\tilde{x}$ such that
$$
\tilde{x} =  \frac{x-prctile(x,PercentileBottom)}{prctile(x,PercentileTop)-prctile(PercentileBottom)}, 
$$
where $prctile(x, Percentile)$ is a function that returns the Percentile% largest value in $x$. Then the pixels with value lower than 0 will be set to 0, and pixels with value larger than 1 will be set to 1.
  |
...
(iv) After image processing with status bar shown in the message box (if select Show process dialog), the denoised (if select Show denoising result) and deconvolved output will pop out in separate Fiji windows automatically. Then the processed images or stacks could be viewed, manipulated, and saved via Fiji.
...
(iii) Click OK to start training. During the training procedure, the training progress and current learning rate will be displayed in a message box, and the model will be validated after each training epoch with the validation input and output shown in another image window for reference. 
(iv) Three types of exit:

+ If you don't want to train or save this model anymore for certain reasons like you got the hyper-parameters wrong, press <i>Cancel > Close</i> to enforce an exit.
+ If you want an early stop, press <i>Finish</i> to finish training progress and save the model by <i>File actions > Save to..</i>.
+ If you have finished training, in <i>Overview > Metadata > inputs & outputs > Training</i>, you will see the parameters of the trained model. Press <i>Export Model</i> and save the model by <i>File actions > Save to..</i>.
